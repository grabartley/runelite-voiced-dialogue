...
1. **Wrong/missing race, gender, or origin accent** (e.g. a woman voiced male, a talkable monster with no race, a foreigner sounding like the locals) -> `tools/overrides.json`.
   - An override **patches** the wiki-inferred entry per field: a field you set wins, a field you omit inherits the wiki value, a field set to `null` is cleared. `race`/`gender` are optional (they inherit), but the entry must end up with both, so set them for ids the wiki does not document. `race` must be one of Human, Elf, Dwarf, Goblin, Gnome, Monkey, Gorilla, Troll, Undead, Demon, Wizard, Tortugan, Icyene, Arceuus, Aranei; map a wiki race that is not one of these (Dorgeshuun -> Goblin, Vampyre -> Undead, Imp -> Demon, ape -> Gorilla, ...). A truly unmappable species (Serpent, Merfolk, ...) gets a `byId` style only.
