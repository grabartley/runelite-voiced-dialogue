...
public final class PredictiveDialoguePrefetcher {
            ...
            clientThread.invokeLater(
                () -> {
                  ...
                  log.debug(
                      ...
                      !successors.isEmpty(),
                      successors.size());
                  offer(successors, expectedNpc, text, currentLineIsNpc);
                }));
  }
  ...
  private void offer(
      List<WikiTranscript.Line> lines,
      String npcName,
      String currentText,
      boolean currentLineIsNpc) {
    ...
    for (WikiTranscript.Line line : lines) {
      ...
      requests.add(player && currentLineIsNpc ? request.withContext(currentText) : request);
    }
