...
public class VoicedDialoguePlugin extends Plugin {
  ...
  /**
   * Detects dialogue on the client tick instead of the 600 ms game tick, so synthesis starts within
   * one UI update. {@link DialogueWatcher} deduplicates unchanged widget text, so the extra scans
   * never re-speak a line.
   */
  @Subscribe
