...
| **Spoken Language** | `English` | Language dialogue is spoken in: type any language name (e.g. `Brazilian Portuguese`, `Japanese`). `English` voices the original line directly; anything else translates each line first (preserving names, places, and item terms), then voices the translation. Adds a translation request per new line. |
