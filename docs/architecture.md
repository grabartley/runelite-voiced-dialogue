...
sub-pool by a stable per-NPC seed so they sound distinct but stable. Age is a third axis: an NPC
marked as a child (a `child` age marker in the bundled table, or a child keyword like "Child" or
"Schoolboy" in the display name) resolves to a dedicated youthful sub-pool of its gender instead of
