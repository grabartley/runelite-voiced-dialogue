...
- A hard pre-build **`smoke` gate** runs first (the full-graph frozen-import check below); the matrix `build` job has `needs: smoke`, so the expensive bundle build only starts once the import recipe is CI-proven clean on a CPU runner. A failing gate means no build, no release, and no asset upload.
...
- **Packaging needs no GPU.** The bundle is assembled by `engine-zonos/packaging/build_bundle.py` on a standard Windows runner: it creates a clean venv with **uv** (`uv venv --python 3.12`, the newest CPython with a torch 2.4.1 cu124 wheel) and installs the PyTorch **CUDA wheels** (which carry their own CUDA runtime) + Zonos + phonemizer via `uv pip install` (uv's parallel resolver + shared cache replaces pip for a faster install with identical contents), downloads the Zonos-v0.1 weights into `model/`, asserts the reference-voice bank under `voices/` is complete for every id the plugin's `ZonosVoiceMap` can emit, then runs **PyInstaller** (`packaging/zonos-engine.spec`) to freeze the engine + its whole dependency graph (embedded interpreter + torch CUDA + Zonos) into `runtime/`. The result is laid out as `zonos-engine(.bat)` launcher + `runtime/` + `voices/` + `model/` + `licenses/` and zipped. Only **running** the bundle's real synthesis needs an NVIDIA GPU, which is the user's machine; CI cannot run real synthesis.
...
- **Full-graph frozen-import gate (`zonos-smoke-test.yml` + the release `smoke` job).** A full Zonos release build is ~50 min and ~2.9 GB, far too expensive to use as the feedback loop for the recipe that decides whether the frozen `import torch` works. The recurring failure — `CPU dispatcher tracer already initlized` / `cannot load module more than once per process` — is a numpy **import-time double-init**, not a CUDA runtime error: it fires the instant the PyInstaller-frozen `import torch` re-initializes numpy's C extension a second time, **before any GPU call** (issue #77 / the `numpy<2.4` pin in `packaging/requirements.txt`). So **no GPU is needed to reproduce it**: a plain CPU runner that freezes the real module graph and runs `import torch` reproduces (or clears) the whole bug class. The smoke freezes the **same module graph as the shipped engine** by calling the **shared collection recipe** in `packaging/_collect.py` (`collect_engine_deps()` — `collect_all` over zonos/phonemizer/numpy + torch/torchaudio data-only) that `zonos-engine.spec` also imports, so the smoke and the real bundle are physically incapable of diverging on the dependency graph (the earlier #99 smoke froze a stripped torch+numpy-only subset and so validated a *different* bundle than shipped). On `windows-latest` with **no GPU**, it `uv pip install`s the full `packaging/requirements.txt` from the **CPU** torch index (cheap, faithful — the double-init is GPU-independent) plus zonos from git, PyInstaller-freezes `packaging/smoke_import.py` via `packaging/smoke-import.spec`, and runs the frozen exe, which does `import torch` + `import zonos` + the engine's real `zonos_engine.synthesizer.cuda_available()`. `cuda_available()` returning False on the GPU-less runner is fine (the double-init surfaces at import, before any CUDA call); it **fails loudly if the frozen import/probe raises or if the output contains the double-load markers**. The release workflow (`zonos-engine-release.yml`) runs this **same freeze as a hard pre-build `smoke` gate** (`build` has `needs: smoke`): a failing import recipe aborts the job **before** the ~2.9 GB build + weight download, so there is no release and no asset upload until the recipe is CI-proven on a CPU runner. `zonos-smoke-test.yml` keeps the same freeze as a standalone PR check on PRs that touch `requirements.txt`, `_collect.py`, either spec, the synthesizer, or the smoke files.
