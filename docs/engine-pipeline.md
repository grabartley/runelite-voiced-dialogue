...
- Each bundle is sha256'd. The `publish` job gathers the shadow jar and every engine target, generates `engine-manifest.json` via `engine-kokoro/scripts/generate_manifest.py`, generates a changelog from the GitHub compare notes, creates the `v<semver>` tag, and publishes a single GitHub Release carrying the shadow jar, the `engine-manifest.json` asset, and the bundles + their `.sha256` files. The manifest is a release asset the plugin fetches at runtime, not a committed resource, so there is no manifest commit-back.
...
1. **Dispatch `CI/CD`** from the Actions tab ("CI/CD" -> "Run workflow"), choosing the `bump` (`patch`/`minor`/`major`) and `release_type` (`alpha`/`beta`/`stable`). One run computes the next `v<semver>`, builds and validates the shadow jar and the three Kokoro bundles, signs the bundles if secrets are present, tags the release, and publishes a single GitHub Release carrying the shadow jar, the `engine-manifest.json` asset, and the bundles. No commit-back: the manifest is a release asset.
2. **Submit/update the plugin in `runelite/plugin-hub`** at the tagged commit (see issue #31) so the Hub builds the thin jar from source at that commit and serves it. The Hub jar is published by the Hub, not by this repo; the shadow jar on the Release is the standalone/sideload artifact.
3. **Users install.** On first use of the local voice, the jar fetches the `engine-manifest.json` for its own version from that Release, downloads the per-OS engine bundle, verifies its sha256, and runs it.
...
The plugin and engine ship under **one version**. A single `v<semver>` tag carries the shadow jar, the engine bundles, and the `engine-manifest.json` asset, and the jar resolves the engine release matching its own stamped version. To cut a new version, dispatch the `CI/CD` deploy with the desired `bump`; it rebuilds and republishes the jar, the engine bundles, and the manifest together under the new tag. Re-tag the plugin commit submitted to the Hub at that release.
