...
def apply_overrides(table, overrides):
    ...
    ``age`` marks a child ("child" is the only value). ``race`` and ``gender`` are optional too,
    ...
    for key, entry in override_npcs.items():
        ...
        for field in ("race", "gender", "ethnicity", "age"):
            ...
            if field == "age" and value not in VALID_AGES:
                raise ValueError(f"Override {key} has invalid age '{value}'")
            merged[field] = value
...
def validate_profiles(profiles):
    ...
    for entry in (profiles.get("byCategory") or []):
        ...
        age = entry.get("age")
        if age is not None and age not in VALID_AGES:
            raise ValueError(f"byCategory entry has invalid age '{age}': {entry!r}")
...
def main():
    ...
    race_counts, gender_counts, ethnicity_counts, age_counts = {}, {}, {}, {}
    for v in table.values():
        ...
        if "age" in v:
            age_counts[v["age"]] = age_counts.get(v["age"], 0) + 1

    out = {
        "_meta": {
            "description": "Static precomputed npcId -> {race, gender, ethnicity?, age?} lookup "
            ...
            "schema": "npcs[id] = { race, gender, ethnicity?, age? }",
    ...
    print(f"  ages:    {out['_meta']['age_counts']}", file=sys.stderr)
